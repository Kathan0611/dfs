...
exports.login = async (req,res) => {
  ...
  try {
    ...
    if ((email || name) && password) {
      ...
      const existUser = await User.find({$or:[{email:email},{name:name}]});
           ...
           console.log(token,"jjjjjjjj")
              if(!token){
                return res.status(400).json({message:"User token is generator "})
                  ...
                  console.log(existUser[0].email,"kk")
                   if(((email===existUser[0].email)||(name===existUser[0].name)) && bcrypt.compareSync(password,existUser[0].password))
                   {
